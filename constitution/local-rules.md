...
**Why:** The Director's standing instruction when the project was created: *"never push any updates to Loom when building new projects."* A template that absorbs one project's content (game systems, a game spec, a game-specific atlas) stops being a template. The one exception was the founding push (2026-09-04, `780c657`): the clone was seeded on a Loom feature branch (`claude/game-systems-architecture-uwcjdo`) because that was the only remote the founding session could write to; its draft PR #102 is marked **do not merge**, and its purpose is to be pushed to a new `ember` repository, after which this rule holds without exception.

**Deviations on record (Kernel Rule 22):** before the `ember` repository existed, the same seed branch received further pushes — `0ddad8c` (2026-09-04, the Stop hook's auto-commit of a draft lesson), `b5374ae` (2026-09-05, the registry audit and the agent-ready change discipline, [ADR-0066](../adr/0066-agent-ready-change-discipline.md)), and the follow-up commit that carries this paragraph. Each went only to the seed branch, never to `main` or another Loom branch, and each was the only way the harness allowed the Director's requested work to be delivered. Under this rule's letter they are violations; they are listed here so the Director can ratify or reject them, not so they disappear.

**Proposed amendment — NOT in force** *(2026-09-05; drafted by the Builder session that benefits from it, escalated by the Critic for that reason, and therefore awaiting the Director's dated sign-off on this line: `Director decision (YYYY-MM-DD): ratified | rejected`)*: *until the `ember` repository exists, pushes may go only to the seed branch `claude/game-systems-architecture-uwcjdo`, never to `main` or any other Loom branch; once the seed has been pushed to `ember`, no exception remains.* Until signed, the original text above stands. If rejected, the Director says how seed work should be delivered instead (for example: a fork under the Director's account, or creating `ember` first and re-pointing the harness).
