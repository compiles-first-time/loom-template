...
**Why:** The Director's standing instruction when the project was created: *"never push any updates to Loom when building new projects."* A template that absorbs one project's content (game systems, a game spec, a game-specific atlas) stops being a template. The one exception is the seed: until the `ember` repository exists, the founding session and its continuations may push **only** to the seed branch `claude/game-systems-architecture-uwcjdo` — the only remote they can write to — whose draft PR is marked **do not merge** and exists to be pushed to the new `ember` repository. Never to `main` or any other Loom branch. Once the seed has been pushed to `ember` (the Director will say so), this rule holds without exception.
