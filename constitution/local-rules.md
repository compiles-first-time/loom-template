...
**Status:** Active
**Date:** 2026-09-04 · **Amended:** 2026-09-05 (seed-branch exception ratified by the Director — see below)
...
**Why:** The Director's standing instruction when the project was created: *"never push any updates to Loom when building new projects."* A template that absorbs one project's content (game systems, a game spec, a game-specific atlas) stops being a template. The first exception was the founding push (2026-09-04, `780c657`): the clone was seeded on a Loom feature branch (`claude/game-systems-architecture-uwcjdo`) because that was the only remote the founding session could write to; its draft PR #102 is marked **do not merge**, and its purpose is to be pushed to a new `ember` repository, after which this rule holds without exception.

**Deviations on record (Kernel Rule 22):** before the `ember` repository existed, the same seed branch received further pushes — `0ddad8c` (2026-09-04, the Stop hook's auto-commit of a draft lesson), `b5374ae` (2026-09-05, the registry audit and the agent-ready change discipline, [ADR-0066](../adr/0066-agent-ready-change-discipline.md)), `a20cb4f` (2026-09-05, the Critic's review applied — this deviation log and the proposal below), `e236da4` (2026-09-05, declared versus observed, [ADR-0067](../adr/0067-declared-versus-observed.md)), and the commit that carries the Director's decision below. Each went only to the seed branch, never to `main` or another Loom branch, and each was the only way the harness allowed the Director's requested work to be delivered. Under the rule's original letter they were violations; they were listed here so the Director could ratify or reject them, not so they disappear. **Ratified by the Director on 2026-09-05** (decision below): they are covered by the amendment and are no longer deviations.

**Amendment — in force** *(proposed 2026-09-05 by the Builder session that benefits from it, escalated by the Critic for that reason, and put to the Director as decision item 1 of PR #102)*: *until the `ember` repository exists, pushes may go only to the seed branch `claude/game-systems-architecture-uwcjdo`, never to `main` or any other Loom branch; once the seed has been pushed to `ember`, no exception remains.*

`Director decision (2026-09-05): ratified` — the Director's words, verbatim, in reply to the escalation: *"if we only have access to the branch, then that is fine."*

*Provenance (Kernel Rule 22):* the reply arrived as a chat message to the Builder session on 2026-09-05, answering the session's report that named this escalation and the deviations above as the first decision awaiting the Director; the session recorded it here and in `docs/changelog.md`, and logged a `director_decision` event. The Director did not type the line above; the session did, quoting the reply. If the Director's intent was narrower than this wording, the Director edits this paragraph and the narrower reading governs from that date. The exception expires by its own terms the moment the seed reaches `ember`.
...
- Any `git push` whose target URL contains `loom-template` is classified `destructive_actions` under LR-04 and requires a `constitution-service` claim — which must refuse, with one exception while the `ember` repository does not yet exist: a fast-forward push to the seed branch `claude/game-systems-architecture-uwcjdo` (the ratified amendment above). Never `main`, never another branch, never a forced push or a history rewrite.
