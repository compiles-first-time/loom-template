...
- **GenAI semantic-convention aliases (added 2026-08-04, inbox item `otel-genai-semconv-alignment`):** events carrying LLM-call fields (`loop_cost_summary`, `session_token_usage` — model + token counts, confirmed present in the event log) now also emit the standard OTel `gen_ai.*` attribute names (`gen_ai.request.model`, `gen_ai.usage.input_tokens`/`output_tokens`) alongside Loom's original keys, so GenAI-aware backends recognize them. **Additive** (originals kept → no backend breaks) and centralized in `GENAI_ALIASES` (`observatory/lib/otel.mjs`) so an upstream rename is a one-line edit — necessary because the GenAI semconv was mid-relocation to its own repo as of 2026-08 and its stability marker is unconfirmed `[otel][M]`.
- **Monitor the monitoring (added 2026-08-03):** when the deferred live-export trigger fires, the pipeline must self-instrument — export-success ratio (`sent/accepted`) as an SLO, plus queue-saturation and refused-points alarms — so audit-trail loss is never silent (Kernel Rule 22 depends on the trail actually arriving). Vendor-neutral principle per Tier-1 AWS OTel-gateway docs `[aws-otel][>95% on the principle]`; signal definition in [L6 §Monitor the monitoring](../layers/L6-observability.md#monitor-the-monitoring). Integration target, consistent with this ADR's deferred-adoption posture.
