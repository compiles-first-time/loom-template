...
| OB-P1B-02 | Cold-start bootstrap fixes (louder banner + `bootstrapped_this_session` marker) | ☑ | BR_09 | advisory (path-2b + hand-authored) in banners + hook; marker persisted from BOTH session-start AND bootstrap scripts (mark-bootstrapped.mjs) + Observatory-visible. critic APPROVE-WITH-FLAGS → both findings fixed → re-review APPROVE |
| OB-LS0-01 | Lessons-service **Phase 0** (frontmatter schema + `loom lessons search` + soft-checks) | ☑ | BR_10 | ADR-0055 Phase 0; zero-infra. Schema + `scripts/lib/lessons.mjs` (search/validate) + wrappers + `lesson-schema` doctor check; all 12 lessons migrated (100% conform); 11 cases = the proposal's 8 acceptance tests |
