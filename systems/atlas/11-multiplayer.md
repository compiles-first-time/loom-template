...
- `replication` **Replication** [P4][spec][orchestrator] — Server events and state mirrored to clients · `core/net/replication/`
  ...
  - `interpolation_prediction` **Interpolation & prediction** [P—][candidate][director] — Client-side prediction of the local player with server reconciliation; snapshot interpolation of remote actors behind a two-tick buffer; nothing extrapolated by default (ADR-0068) · `core/net/replication/prediction.gd` +1
...
flowchart LR
  ...
  net_architecture -- "3" --> replication
...
| `interpolation_prediction` Interpolation & prediction | 3 | director | Client-side prediction of the local player with server reconciliation; snapshot interpolation of remote actors behind a two-tick buffer; nothing extrapolated by default (ADR-0068) |
...
23 outbound (a change here reaches out), 42 inbound (a change elsewhere reaches in).
...
| ← in | `pure_movement_step` → `interpolation_prediction` | reads | `re-simulation` | hard | Prediction re-runs the same step the server runs |
...
| → out | `interpolation_prediction` → `g6_feel_gate` | validates | `error under latency` | hard | The gate measures prediction error and corrections under a simulated network |
