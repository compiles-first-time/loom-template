...
- `validation_gates` **Validation gates** [P0][spec][test-pilot/orchestrator] — G0 to G5 plus determinism and performance checks · `.github/workflows/gates.yml` +2
  ...
  - `g6_feel_gate` **G6 feel gate** [P—][candidate][test-pilot] — Scripted movement run on the benchmark scene: tick cost, input-to-sim latency, determinism replay and prediction error under simulated latency headless; frame pacing, hitches and tick-locked visuals on the reference machine (ADR-0068) · `tools/testing/feel.gd` +1
...
flowchart LR
  ...
  hardware_scalability -- "4" --> validation_gates
  ...
  camera -- "1" --> validation_gates
  replication -- "1" --> validation_gates
...
| `g6_feel_gate` G6 feel gate | 3 | test-pilot | Scripted movement run on the benchmark scene: tick cost, input-to-sim latency, determinism replay and prediction error under simulated latency headless; frame pacing, hitches and tick-locked visuals on the reference machine (ADR-0068) |
...
29 outbound (a change here reaches out), 56 inbound (a change elsewhere reaches in).
...
| ← in | `locomotion` → `g6_feel_gate` | validates | `latency, determinism` | hard | The gate measures input-to-sim ticks and replays a recorded walk |
| ← in | `third_person_rig` → `g6_feel_gate` | validates | `frame pacing` | hard | The gate measures frame time and hitches during the camera soak |
| ← in | `interpolation_prediction` → `g6_feel_gate` | validates | `error under latency` | hard | The gate measures prediction error and corrections under a simulated network |
| ← in | `render_interpolation` → `g6_feel_gate` | validates | `tick-locked visuals` | hard | The gate fails a transform that only changes on ticks |
| ← in | `pure_movement_step` → `determinism_replay_tests` | validates | `replay` | hard | Replay tests are the proof that the step is pure |
