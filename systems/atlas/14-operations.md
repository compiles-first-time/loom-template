...
[P0][spec][test-pilot] · Gates, CI, release, monitoring, hardware scalability, security, documentation, agent harness
...
- `validation_gates` **Validation gates** [P0][spec][test-pilot] — G0 to G5 plus determinism and performance checks · `.github/workflows/gates.yml` +2
  ...
  - `g2_data_integrity` **G2 data integrity** [P0][spec][test-pilot] — Ids, references, paths, enums and ranges · `tools/validate_data.gd`
...
- `release_distribution` **Release & distribution** [P1][implied][orchestrator] — Versioning, builds, server distribution, and candidate patching, Steam and mods · `export_presets.cfg` +1
  ...
  - `versioning_changelog` **Versioning & changelog** [P0][spec][orchestrator] — One changelog line per task; version tags · `docs/changelog.md`
  - `desktop_builds_win_linux` **Desktop builds** [P1][spec][orchestrator] — Windows and Linux builds · `export_presets.cfg`
  - `dedicated_server_distribution` **Server distribution** [P4][implied][orchestrator] — Shipping the headless server · `server/`
  - `patching_updates` **Patching & updates** [P—][candidate][director] — Delivering updates to players · `docs/`
  - `steam_platform` **Steam platform** [P—][candidate][director] — Steamworks integration · `tools/`
...
- `monitoring_telemetry` **Monitoring & telemetry** [P1][implied][orchestrator] — Client overlays, server metrics, logs, crashes, alerts, analytics · `server/metrics` +1
  - `client_perf_overlay` **Client performance overlay** [P1][implied][orchestrator] — Frame time, draw calls and memory on screen · `core/debug/overlays.gd`
...
- `hardware_scalability` **Hardware range & scalability** [P1][implied][orchestrator] — Specs, presets, scaling, fallbacks, benchmark, frame budget · `project.godot` +1
  ...
  - `min_recommended_specs` **Minimum and recommended specs** [P1][implied][director] — The hardware the game targets · `docs/`
  ...
  - `benchmark_scene` **Benchmark scene** [P—][candidate][test-pilot] — A fixed scene for measuring · `scenes/benchmark.tscn`
...
- `documentation` **Documentation** [P0][spec][director/orchestrator] — The spec, changelog, tech debt, migrations, engine version, the atlas · `docs/`
  ...
  - `changelog` **Changelog** [P0][spec][orchestrator] — One line per task · `docs/changelog.md`
  ...
  - `engine_version_doc` **Engine version doc** [P0][spec][director] — The pinned engine version · `docs/ENGINE_VERSION.txt`
...
- `agent_harness` **Agent harness** [P0][spec][orchestrator] — Skills, write scopes, the Godot MCP bridge, Loom's governance layer · `.claude/` +2
  ...
  - `write_scope_enforcement` **Write scope enforcement** [P0][spec][orchestrator] — The orchestrator rejects diffs outside a role's write scope · `.claude/` +1
...
| `patching_updates` Patching & updates | 3 | director | Delivering updates to players |
...
24 outbound (a change here reaches out), 50 inbound (a change elsewhere reaches in).
...
| ← in | `loot` → `g2_data_integrity` | validates | `loot tables` | hard | G2 checks every table |
...
| ← in | `quests` → `g2_data_integrity` | validates | `quest files` | hard | G2 checks every quest and its target ids |
| ← in | `dialogue` → `g2_data_integrity` | validates | `dialogue files` | hard | G2 checks every dialogue |
| ← in | `crafting` → `g2_data_integrity` | validates | `recipes` | hard | G2 checks every recipe |
...
| ← in | `gray_box_island` → `g3_smoke_boot` | validates | `main scene` | hard | Smoke boots the main scene |
...
| ← in | `gathering` → `g4_bot_playtest` | validates | `gather` | hard | The bot gathers |
| ← in | `crafting` → `g4_bot_playtest` | validates | `craft` | hard | The bot crafts |
...
| ← in | `dedicated_server_build` → `dedicated_server_distribution` | reads | `build` | hard | The server ships as its build |
...
| ← in | `data_dir_as_db` → `mod_support` | reads | `mod data` | hard | Mods are data in the same store |
| ← in | `content_pipeline` → `mod_support` | reads | `same pipeline` | hard | Mods enter through the same pipeline |
| ← in | `debug_overlays` → `client_perf_overlay` | reads | `overlay` | hard | The overlay is a debug overlay |
| ← in | `server_metrics_hooks` → `server_health_metrics` | reads | `hooks` | hard | Metrics read the server hooks |
...
| ← in | `gameplay_analytics_events` → `gameplay_analytics` | reads | `events` | hard | Analytics aggregate stored events |
| ← in | `fixed_tick_sim` → `frame_time_budget` | reads | `tick cost` | hard | The sim tick is part of the budget |
| ← in | `graphics_presets` → `graphics_quality_presets` | extends | `presets` | hard | The presets are the settings presets |
...
| ← in | `godot_version_pin` → `engine_version_doc` | reads | `version` | hard | The doc records the pin |
