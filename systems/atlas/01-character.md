...
- `attributes_stats` **Attributes & stats** [P1][implied][orchestrator] — The numbers: derived stats from gear and effects, resistances, level scaling · `core/stats/`
  ...
  - `level_scaling` **Level scaling** [P2][implied][orchestrator] — Stat growth per level for players and enemies; the curves live in docs/balance_ranges.md · `core/stats/scaling.gd` +1
...
- `class_resources` **Class resources** [P1][spec][orchestrator] — Health, mana, stamina pools and their regeneration; other pools are candidates · `core/stats/resources.gd`
  ...
  - `additional_class_resources` **Further class resources** [P—][candidate][director] — Energy, rage, focus, divinity and corruption as further pools; each is a §6.3 cost enum value and one DIRECTOR decision · `core/stats/resources.gd`
...
- `equipment` **Equipment & gear** [P1][spec][orchestrator] — Equip slots, gear stat application, tiers and comparison · `core/inventory/equipment.gd`
  ...
  - `gear_tiers` **Gear tiers** [P2][spec][content-smith] — Progression bands of gear power tied to rarity and biome · `docs/balance_ranges.md` +1
...
- `character_identity` **Character identity & lifecycle** [P1][implied][orchestrator] — Creation, appearance, death and respawn, and the saved character record · `core/saving/` +1
  ...
  - `revive_downed_state` **Downed & revive** [P—][candidate][director] — A downed state that allies can revive before death; the hardcore-stakes question alongside loot_bags_on_death · `core/combat/downed.gd`
...
flowchart LR
  ...
  loot -- "1" --> inventory
  player_controller -- "1" --> inventory
  quests -- "1" --> character_identity
  needs -- "1" --> character_identity
  abilities_skills -- "1" --> character_identity
  ...
  player_controller -- "2" --> validation_gates
```
...
|---|---|---|---|
| `revive_downed_state` Downed & revive | 3 | director | A downed state that allies can revive before death; the hardcore-stakes question alongside loot_bags_on_death |
...
| `additional_class_resources` Further class resources | 3 | director | Energy, rage, focus, divinity and corruption as further pools; each is a §6.3 cost enum value and one DIRECTOR decision |
...
76 outbound (a change here reaches out), 61 inbound (a change elsewhere reaches in).
...
| → out | `item_pickup_drop` → `gm_console` | reads | `give` | hard | `give <item_id> [n]` puts items in the bag through the pickup path; §13 puts the command in P0 and inventory in P2 — DIRECTOR: stub or move |
...
| ← in | `schema_spell_def` → `additional_class_resources` | reads | — | hard | The cost.resource enum must grow before a spell can cost a new resource |
| ← in | `loot_rolls_on_death` → `item_pickup_drop` | reads | `dropped stacks` | hard | Rolled drops become pickups at the corpse |
| ← in | `equipment_items` → `equip_slots` | reads | `slot != none` | hard | Only items with a slot other than none equip |
| ← in | `quests` → `character_save_state` | persists | `quest state` | hard | Started, completed and objective progress per character |
| ← in | `needs` → `character_save_state` | persists | — | hard | Hunger and the other needs are saved per character |
| ← in | `recipe_unlocks` → `character_save_state` | persists | — | hard | Known recipes are saved per character |
| ← in | `party_membership` → `xp_award` | reads | — | soft | Kill XP is shared with the party |
| ← in | `death_resolution` → `revive_downed_state` | reads | — | hard | Downed sits between zero health and death |
| ← in | `pure_movement_step` → `locomotion` | reads | `pure step` | hard | Locomotion is written as the pure step so replay and prediction can re-run it |
...
| → out | `additional_class_resources` → `resource_cost_check` | reads | — | soft | The cost check would read the new pools |
| → out | `spellbook` → `cast_timing` | reads | `known spell ids` | hard | Only known spells cast |
...
| → out | `interaction_system` → `resource_nodes` | reads | `interact target id` | hard | Harvesting is an interact on a node |
...
| → out | `interaction_system` → `chests_containers` | reads | `interact target id` | hard | Opening a container is an interact |
...
| → out | `interaction_system` → `dialogue_runner` | reads | `interact target id` | hard | Talking is an interact on an NPC |
...
| → out | `gear_stats_application` → `tooltips_item_cards` | reads | — | hard | Comparison deltas come from the equipped item's applied stats |
...
| → out | `progression` → `sync_domains` | transports | — | hard | XP and levels are server-owned; no dedicated sync part yet |
| → out | `equipment` → `sync_domains` | transports | — | hard | Equipped state replicates from the server; no dedicated sync part yet |
...
| → out | `character_save_state` → `cross_server_characters` | reads | — | hard | A character that moves between servers is its saved state |
| → out | `character_save_state` → `per_player_save_in_world` | reads | — | hard | Each player's record inside the world save |
| → out | `character_save_state` → `character_migration_between_worlds` | reads | — | hard | Migration moves the saved state |
...
| → out | `locomotion` → `g6_feel_gate` | validates | `latency, determinism` | hard | The gate measures input-to-sim ticks and replays a recorded walk |
