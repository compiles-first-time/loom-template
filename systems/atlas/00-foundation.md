...
- `engine_platform` **Engine & Build** [P0][spec][orchestrator] — Godot 4.x pinned, project settings, export presets, addon policy · `project.godot` +2
  ...
  - `export_presets` **Export presets** [P1][spec][orchestrator] — Windows and Linux desktop builds, plus the headless server preset in Phase 4 · `export_presets.cfg`
...
- `architecture_rules` **Architecture rules (R1–R10)** [P0][spec][orchestrator] — The constitution of the codebase, enforced by gates and by refusal · `GAME_INFRA_SPEC.md` +1
  ...
  - `deterministic_sim` **Deterministic simulation (R4)** [P1][spec][orchestrator] — Seeded RNG passed in, fixed physics tick, no wall-clock inside core/ · `core/`
...
- `event_bus` **EventBus** [P0][spec][orchestrator] — The autoload every system emits on and listens to; its signal table is the API between systems · `core/events/event_bus.gd`
  ...
  - `sig_structure_placed` **structure_placed (proposed)** [P3][candidate][orchestrator] — actor_id, structure_id, position; building has no signal in §5 yet · `core/events/event_bus.gd`
  - `sig_structure_destroyed` **structure_destroyed (proposed)** [P3][candidate][orchestrator] — structure_id, cause · `core/events/event_bus.gd`
  - `sig_player_joined` **player_joined (proposed)** [P4][candidate][orchestrator] — player_id, character_id · `core/events/event_bus.gd`
  - `sig_player_left` **player_left (proposed)** [P4][candidate][orchestrator] — player_id, reason · `core/events/event_bus.gd`
  - `sig_level_up` **level_up (proposed)** [P2][candidate][orchestrator] — actor_id, level · `core/events/event_bus.gd`
  ...
  - `sig_boss_phase_changed` **boss_phase_changed (proposed)** [P2][candidate][orchestrator] — encounter_id, phase_index · `core/events/event_bus.gd`
  - `sig_weather_changed` **weather_changed (proposed)** [P—][candidate][orchestrator] — weather_id, intensity · `core/events/event_bus.gd`
  - `sig_zone_entered` **zone_entered (proposed)** [P3][candidate][orchestrator] — actor_id, zone_id · `core/events/event_bus.gd`
  - `sig_need_threshold_crossed` **need_threshold_crossed (proposed)** [P3][candidate][orchestrator] — actor_id, need_id, threshold; how survival asks effects for a starving debuff without calling it · `core/events/event_bus.gd`
...
- `data_schemas` **Data schemas (the nouns)** [P0][spec][orchestrator] — Resource class definitions every content file must match · `core/schemas/`
  - `schema_conventions` **Schema conventions** [P0][spec][orchestrator] — Every def carries id, display_name, schema_version; canonical storage is .tres · `core/schemas/`
  - `schema_versioning` **Schema versioning + migrations** [P2][spec][orchestrator] — schema_version bumps with a migration note so old saves and old data still load · `core/schemas/` +1
  ...
  - `schema_biome_def` **BiomeDef (implied)** [P1][implied][orchestrator] — data/biomes exists in the layout but has no schema section yet · `core/schemas/biome_def.gd`
  ...
  - `schema_station_def` **StationDef (implied)** [P3][implied][orchestrator] — RecipeDef.station names a station id (hands, workbench, forge) · `core/schemas/station_def.gd`
...
- `content_pipeline` **Content pipeline** [P0][spec][orchestrator] — inbox JSON → converter → validator → data/ .tres, with hot-reload at runtime · `tools/json_to_tres.gd` +2
  ...
  - `data_validator_g2` **Data validator (G2)** [P0][spec][test-pilot] — Unique ids, references resolve, icon and model paths exist, enums legal, numbers in range · `tools/validate_data.gd`
...
- `time_and_tick` **Time & scheduling** [P1][spec][orchestrator] — Fixed simulation tick, timers and cooldowns, pause rules · `core/`
  - `fixed_tick_sim` **Fixed-tick simulation loop** [P1][spec][orchestrator] — Gameplay advances on the physics tick; all durations count ticks, not seconds · `core/`
  - `timers_cooldowns` **Timers & cooldowns** [P1][implied][orchestrator] — Tick-based timers shared by cooldowns, craft time, respawns, effect durations · `core/`
  - `pause_rules` **Pause & game speed** [P1][implied][orchestrator] — Single-player pause; no pause once a server owns time in Phase 4 · `core/`
- `command_intents` **Command / intent events** [P1][spec][orchestrator] — Player actions enter the sim as intents, the shape a server will receive later · `core/commands/`
  - `intent_schema` **Intent schema** [P1][spec][orchestrator] — Typed intents: move, interact, cast spell_id at target, use item, build piece · `core/commands/`
  - `intent_dispatch` **Intent dispatch** [P1][spec][orchestrator] — Intents are queued and applied on the tick by the owning system · `core/commands/`
  - `intent_validation` **Intent validation** [P1][implied][orchestrator] — Illegal intents are rejected before they touch state; the server does this in Phase 4 · `core/commands/`
...
- `debug_tools` **Debug & GM tools** [P0][spec][orchestrator] — The console and overlays that make the content loop fast · `core/debug/console.gd`
  ...
  - `debug_overlays` **Debug overlays** [P1][implied][orchestrator] — Frame time, actor ids, threat, navmesh and hitbox drawing · `core/debug/overlays.gd`
  ...
  - `replay_recorder` **Input replay recorder** [P—][candidate][test-pilot] — Determinism makes intent logs replayable: record once, reproduce any bug · `core/debug/replay.gd`
- `config_settings` **Settings & configuration** [P1][implied][orchestrator] — User settings store and the presets other systems read · `core/settings/` +1
  ...
  - `graphics_presets` **Graphics presets** [P1][implied][orchestrator] — Low, medium, high bundles that map to renderer switches · `core/settings/graphics.gd`
...
flowchart LR
  ...
  time_and_tick -- "1" --> loot
  state_model -- "1" --> loot
  data_schemas -- "2" --> crafting
  time_and_tick -- "1" --> crafting
  command_intents -- "1" --> crafting
  architecture_rules -- "2" --> crafting
  architecture_rules -- "1" --> professions
  architecture_rules -- "1" --> currencies
  ...
  architecture_rules -- "2" --> live_events
  ...
  engine_platform -- "1" --> documentation
...
| `replay_recorder` Input replay recorder | 3 | test-pilot | Determinism makes intent logs replayable: record once, reproduce any bug |
...
276 outbound (a change here reaches out), 41 inbound (a change elsewhere reaches in).
...
| ← in | `day_night_cycle` → `gm_console` | reads | `time command` | soft | The time command sets the clock phase; the spec lists the console with time in Phase 0 but the clock in Phase 3, so Phase 0 needs a stub clock or the clock moves earlier (DIRECTOR) |
...
| → out | `schema_spell_def` → `energy_pool` | reads | `cost.resource enum` | hard | The enum must grow before a spell can cost energy |
| → out | `schema_spell_def` → `rage_pool` | reads | `cost.resource enum` | hard | The enum must grow before a spell can cost rage |
| → out | `schema_spell_def` → `focus_pool` | reads | `cost.resource enum` | hard | The enum must grow before a spell can cost focus |
| → out | `schema_spell_def` → `divinity_pool` | reads | `cost.resource enum` | hard | The enum must grow before a spell can cost divinity |
| → out | `schema_spell_def` → `corruption_pool` | reads | `cost.resource enum` | hard | The enum must grow before a spell can cost corruption |
...
| → out | `timers_cooldowns` → `world_chests_containers` | reads | `respawn` | soft | Chests can refill on a timer |
...
| → out | `id_convention` → `profession_defs` | reads | `profession ids` | hard | Professions would carry ids like all content |
| → out | `id_convention` → `currency_defs` | reads | `currency ids` | hard | Currencies would carry ids like all content |
...
| → out | `actor_registry` → `actor_ids` | extends | `id space` | hard | Actor ids are entries in the registry |
...
| → out | `project_settings` → `physics_tick_settings` | extends | `tick rate` | hard | The tick rate is a project setting |
...
| → out | `id_convention` → `holiday_defs` | reads | `ids` | hard | Holidays would carry ids like all content |
...
| → out | `graphics_presets` → `settings_screens` | reads | `presets` | hard | The graphics tab offers presets |
...
| → out | `user_settings_store` → `server_config_password` | reads | `host settings` | soft | The host edits these as settings |
...
| → out | `id_convention` → `save_ids_plus_state` | reads | `ids` | hard | The rule depends on stable ids |
| → out | `state_serialization` → `save_ids_plus_state` | reads | `state shape` | hard | The serializer enforces ids plus state |
...
| → out | `content_pipeline` → `data_dir_as_db` | reads | `filed .tres` | hard | The pipeline files content into data/ |
| → out | `plain_text_formats` → `data_dir_as_db` | reads | `text only` | hard | Git as a database works because content is text |
| → out | `id_convention` → `id_immutability_migrations` | reads | `ids` | hard | The rule is about ids |
| → out | `event_bus` → `gameplay_analytics_events` | reads | `all signals` | hard | Analytics subscribe to the bus |
...
| → out | `debug_overlays` → `client_perf_overlay` | reads | `overlay` | hard | The overlay is a debug overlay |
...
| → out | `graphics_presets` → `graphics_quality_presets` | extends | `presets` | hard | The presets are the settings presets |
...
| → out | `godot_version_pin` → `engine_version_doc` | reads | `version` | hard | The doc records the pin |
