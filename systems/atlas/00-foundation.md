...
- `architecture_rules` **Architecture rules (R1–R10)** [P0][spec][orchestrator] — The constitution of the codebase, enforced by gates and by refusal · `GAME_INFRA_SPEC.md` +1
  ...
  - `pure_movement_step` **Pure movement step (proposed R11)** [P—][candidate][orchestrator] — Movement is a pure tick step (state, intent, tick) to state: no frame time, node paths or wall clock inside; re-runnable for replay and client prediction (ADR-0068) · `actors/player/locomotion.gd` +1
...
- `time_and_tick` **Time & scheduling** [P1][spec][orchestrator] — Fixed simulation tick, timers and cooldowns, pause rules · `core/time/`
  ...
  - `render_interpolation` **Render interpolation** [P—][candidate][orchestrator] — Physics interpolation on: visuals read transforms interpolated between 60 Hz ticks; jitter fix 0; a teleport resets interpolation (ADR-0068) · `project.godot` +1
...
flowchart LR
  ...
  engine_platform -- "2" --> time_and_tick
  ...
  architecture_rules -- "1" --> player_controller
  ...
  architecture_rules -- "1" --> replication
  ...
  architecture_rules -- "4" --> validation_gates
  ...
  time_and_tick -- "1" --> validation_gates
...
| `render_interpolation` Render interpolation | 3 | orchestrator | Physics interpolation on: visuals read transforms interpolated between 60 Hz ticks; jitter fix 0; a teleport resets interpolation (ADR-0068) |
...
| `pure_movement_step` Pure movement step (proposed R11) | 3 | orchestrator | Movement is a pure tick step (state, intent, tick) to state: no frame time, node paths or wall clock inside; re-runnable for replay and client prediction (ADR-0068) |

## Cross-domain edges

293 outbound (a change here reaches out), 51 inbound (a change elsewhere reaches in).
...
| → out | `pure_movement_step` → `locomotion` | reads | `pure step` | hard | Locomotion is written as the pure step so replay and prediction can re-run it |
...
| → out | `render_interpolation` → `third_person_rig` | reads | `interpolated target transform` | hard | The camera follows the interpolated transform every frame, never the raw tick position |
| → out | `render_interpolation` → `locomotion_blending` | reads | `frame-rate playback` | soft | Blends play at frame rate over interpolated motion; tick-locked playback looks choppy |
...
| → out | `pure_movement_step` → `interpolation_prediction` | reads | `re-simulation` | hard | Prediction re-runs the same step the server runs |
...
| → out | `render_interpolation` → `g6_feel_gate` | validates | `tick-locked visuals` | hard | The gate fails a transform that only changes on ticks |
| → out | `pure_movement_step` → `determinism_replay_tests` | validates | `replay` | hard | Replay tests are the proof that the step is pure |
