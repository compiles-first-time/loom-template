...
- `camp_rest` **Camp & rest** [P3][implied][orchestrator] — Fire, bed and cooking: the loop that makes survival a home · `core/survival/` +1
  - `campfire` **Campfire** [P3][implied][content-smith] — A placeable station that gives warmth, light and cooking · `data/building/`
  ...
  - `cooking` **Cooking** [P3][implied][content-smith] — Recipes at the campfire station that produce food · `data/recipes/`
...
21 outbound (a change here reaches out), 44 inbound (a change elsewhere reaches in).

| Direction | Edge | How | Via | Strength | Why |
|---|---|---|---|---|---|
| → out | `day_night_cycle` → `gm_console` | reads | `time command` | soft | The time command sets the clock phase; the spec lists the console with time in Phase 0 but the clock in Phase 3, so Phase 0 needs a stub clock or the clock moves earlier (DIRECTOR) |
...
| ← in | `crafting_station_structures` → `campfire` | extends | `station piece` | hard | A campfire is a placeable station |
...
| ← in | `recipe_defs` → `cooking` | reads | `station campfire` | hard | Cooking is a set of recipes at the campfire station |
...
| → out | `gathering` → `g4_bot_playtest` | validates | `gather` | hard | The bot gathers |
