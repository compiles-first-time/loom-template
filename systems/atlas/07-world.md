...
[P1][spec][world-builder/orchestrator] · Terrain and biomes, generation, environment, travel, places, physics, navigation
...
- `terrain_biomes` **Terrain & biomes** [P1][spec][world-builder] — Biomes and the terrain, vegetation and water that make them · `scenes/` +1
  - `biome_defs` **Biome definitions** [P0][spec][world-builder] — One def per biome; Phase 0 ships one stub so the sample EnemyDef.biomes resolves (G2) · `data/biomes/`
  ...
  - `vegetation_placement` **Vegetation placement** [P1][implied][world-builder] — Trees and grass placed per biome · `scenes/terrain/`
  - `biome_palette_lighting` **Biome palette & light rig** [P1][implied][world-builder] — Each biome's palette slice and light rig · `art/` +1
  - `water_bodies` **Water bodies** [P—][candidate][world-builder] — Lakes, rivers and sea with swim and drown volumes · `scenes/terrain/`
  - `caves` **Caves** [P—][candidate][world-builder] — Underground spaces · `scenes/terrain/`
...
- `environment` **Environment** [P1][implied][world-builder/orchestrator] — Lighting, fog, weather and seasons · `scenes/env/` +1
...
- `travel` **Travel** [P1][implied][orchestrator] — Terrain rules, zones, markers, maps, and the candidate fast travel, roads, mounts, boats · `core/world/` +1
  ...
  - `roads` **Roads** [P—][candidate][world-builder] — Paths that speed travel and guide players · `scenes/terrain/`
  ...
  - `scene_transitions_loading` **Scene transitions & loading** [P2][implied][orchestrator] — Loading and switching scenes — entering a dungeon, a loading screen — while actor state crosses the switch as data · `core/world/scene_loader.gd` +1
- `places` **Places** [P1][implied][world-builder] — Villages, dungeon entrances, hubs, landmarks, and candidate cities · `scenes/` +1
  ...
  - `dungeon_entrances` **Dungeon entrances** [P2][implied][world-builder] — Where dungeons connect to the world · `scenes/prefabs/`
  - `spawn_hubs` **Spawn hubs** [P1][implied][world-builder] — Default player spawn locations · `scenes/markers/`
  - `landmarks_poi` **Landmarks** [P1][implied][world-builder] — Visual anchors for navigation · `scenes/prefabs/`
...
- `physics_collision` **Physics & collision** [P1][spec][orchestrator] — Collision generation, layers and tick settings · `tools/import_post.gd` +1
  ...
  - `destructibles` **Destructibles** [P—][candidate][director] — Breakable props · `scenes/prefabs/`
- `navigation` **Navigation** [P1][implied][world-builder/orchestrator] — Navmesh and links the AI walks on · `scenes/`
  - `navmesh_baking` **Navmesh baking** [P1][implied][world-builder] — Baked navigation regions, rebaked when geometry changes · `scenes/terrain/nav/` +1
  - `nav_regions_links` **Nav regions & links** [P1][implied][world-builder] — Regions and links across gaps · `scenes/terrain/nav/`
...
flowchart LR
  ...
  physics_collision -- "1" --> terrain_biomes
  navigation -- "1" --> terrain_biomes
  architecture_rules -- "1" --> world_generation
  terrain_biomes -- "2" --> world_generation
...
| `world_generation` World generation | 2 (+5 parts) | director | Procedural terrain from a seed versus the hand-built world the spec starts with |
...
44 outbound (a change here reaches out), 34 inbound (a change elsewhere reaches in).
...
| → out | `navmesh_baking` → `debug_overlays` | renders | — | soft | The navmesh overlay draws the baked mesh |
| ← in | `sig_weather_changed` → `fog_atmosphere` | listens | — | soft | Fog density follows the weather |
...
| ← in | `import_hook` → `navmesh_baking` | reads | — | hard | The navmesh bakes on the collision the import hook generated |
| ← in | `actor_prefabs_scenes` → `mounts_travel` | reads | — | hard | A mount is an actor prefab the player rides |
| ← in | `state_serialization` → `scene_transitions_loading` | reads | — | soft | From Phase 3 actor state crosses a scene switch as serialized data; before that it stays in memory |
...
| → out | `collision_layers` → `third_person_rig` | reads | — | hard | The camera pulls in when geometry blocks the view |
...
| → out | `gray_box_island` → `g3_smoke_boot` | validates | — | soft | Phase 0 boots an empty project; the island is what boots from Phase 1 |
