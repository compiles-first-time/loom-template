...
- `boss_encounters` **Boss encounters** [P2][spec][orchestrator/content-smith] — Data-defined fights with phases and mechanics; beating one opens a gate · `core/encounters/` +1
  ...
  - `raid_bosses` **Raid bosses** [P4][implied][orchestrator] — Bosses tuned for the whole server · `core/encounters/`
...
- `raids` **Raids** [P4][implied][orchestrator] — Encounters sized for the whole server · `core/encounters/raid.gd`
  - `raid_size_scaling` **Raid size scaling** [P4][implied][orchestrator] — Health and mechanics scale with player count · `core/encounters/raid.gd`
  - `raid_roles_check` **Raid roles check** [P4][implied][orchestrator] — Warns when the group lacks a role · `core/encounters/raid.gd`
...
- `world_events` **World events** [P—][candidate][director] — Invasions and timed events that change the world for a while · `core/events_world/`
  ...
  - `dynamic_events_ai_director` **Director-driven events** [P—][candidate][orchestrator] — Events fired by the AI director's pacing · `core/ai/director.gd`
...
|---|---|---|---|
| `world_events` World events | 2 (+3 parts) | director | Invasions and timed events that change the world for a while |
| `dynamic_events_ai_director` Director-driven events | 3 | orchestrator | Events fired by the AI director's pacing |
...
9 outbound (a change here reaches out), 35 inbound (a change elsewhere reaches in).
...
| ← in | `ai_director` → `dynamic_events_ai_director` | reads | `pacing` | hard | The director decides when to fire |
...
| → out | `raid_size_scaling` → `raid_groups` | reads | `size` | hard | Raid size feeds scaling |
