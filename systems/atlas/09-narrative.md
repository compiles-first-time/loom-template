...
[P2][spec][quest-writer] · Quests, dialogue, lore, onboarding, live events, in-engine story moments
...
- `onboarding_tutorial` **Onboarding & tutorial** [P—][candidate][quest-writer] — The first hour: tutorial quests and just-in-time hints · `data/quests/` +1
...
| `onboarding_tutorial` Onboarding & tutorial | 2 (+2 parts) | quest-writer | The first hour: tutorial quests and just-in-time hints |
...
| ← in | `npc_defs` → `quest_defs` | references | `QuestDef.giver_npc` | hard | A quest names its giver |
...
| ← in | `npc_defs` → `objective_types` | references | `talk target_id` | hard | A talk objective names an NPC |
...
| ← in | `id_convention` → `holiday_defs` | reads | `ids` | hard | Holidays would carry ids like all content |
...
| → out | `quests` → `g2_data_integrity` | validates | `quest files` | hard | G2 checks every quest and its target ids |
| → out | `dialogue` → `g2_data_integrity` | validates | `dialogue files` | hard | G2 checks every dialogue |
...
| → out | `holiday_defs` → `seasonal_drops` | reads | `events` | hard | Drops are tied to events |
