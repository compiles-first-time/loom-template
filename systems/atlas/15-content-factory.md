...
- `agent_content_loop` **Agent content loop** [P2][spec][content-smith] — Request to JSON to converter to gates to PR; the loop the architecture exists for · `data/_inbox/`
  ...
  - `converter_gate_run` **Converter and gate run** [P2][spec][content-smith] — Run the converter, then G2, then G1 · `data/_inbox/` +1
  - `pr_review_flow` **PR review flow** [P2][spec][orchestrator] — The PR with .tres files, inbox JSON removed, ids listed, gates pasted · `.github/pull_request_template.md`
- `balance_tuning` **Balance & tuning** [P2][spec][content-smith/director] — Ranges, curves, difficulty, and candidate simulations · `docs/balance_ranges.md` +1
  ...
  - `loot_economy_sim` **Loot economy simulation** [P—][candidate][test-pilot] — Monte Carlo over loot tables · `tools/testing/balance/loot_sim.gd`
  - `combat_sim_harness` **Combat simulation harness** [P—][candidate][test-pilot] — Headless fights over many seeds · `tools/testing/balance/combat_sim.gd`
- `asset_generation` **Asset generation** [P2][spec][world-builder/content-smith] — Icon and model generation through scripts and the import pipeline · `tools/gen/` +1
  ...
  - `icon_placeholder_fallback` **Icon placeholder fallback** [P0][spec][content-smith] — Use the placeholder until the icon exists · `art/icons/_placeholder.png`
- `content_review` **Content review** [P1][spec][test-pilot] — Screenshot sets, lore voice, id listing, flaky-test policy · `tests/` +1
  ...
  - `id_listing_in_pr` **Id listing in PR** [P0][spec][orchestrator] — New ids listed in every content PR · `.github/pull_request_template.md`
  - `flaky_test_policy` **Flaky test policy** [P0][spec][test-pilot] — A test that would not pass 100 of 100 does not merge · `.github/workflows/gates.yml` +1
...
flowchart LR
  ...
  content_review -- "1" --> agent_content_loop
```
...
|---|---|---|---|
| `live_content_ops` Live content operations | 2 (+1 parts) | director | Patches, seasonal drops, data-only hotfixes |
...
18 outbound (a change here reaches out), 37 inbound (a change elsewhere reaches in).
...
| → out | `icon_placeholder_fallback` → `data_validator_g2` | reads | — | soft | G2 accepts the placeholder path as existing |
...
| ← in | `content_database_git` → `hotfix_data_only` | reads | — | hard | A data-only hotfix is a data/ commit |
| ← in | `lore_additions` → `lore_voice_check` | reads | — | soft | New lore is checked against the voice |
| ← in | `tech_debt_log` → `pr_review_flow` | reads | — | soft | R9: unrelated debt is filed, not fixed |
