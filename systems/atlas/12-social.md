...
- `groups_parties` **Groups & parties** [P4][implied][orchestrator] — Membership, roles, loot rules, raid groups · `core/social/party.gd`
  ...
  - `party_loot_rules` **Party loot rules** [P4][implied][orchestrator] — The party's chosen loot rule · `core/social/party.gd`
  - `raid_groups` **Raid groups** [P4][implied][orchestrator] — Full-server groups for raids · `core/social/party.gd`
...
- `reputation_factions` **Reputation & factions** [P—][candidate][director] — Factions, standing tiers, sources, unlocks, hostility, alignment · `data/factions/` +1
  ...
  - `mob_faction_hostility` **Mob faction hostility** [P—][candidate][director] — Standing decides whether mobs attack · `core/ai/`
...
- `community_admin` **Community & admin** [P4][implied][orchestrator] — House rules, friends, blocking · `server/`
  - `server_rules_config` **Server rules** [P4][implied][orchestrator] — PvP off, friendly fire, loot rule defaults · `server/config`
...
flowchart LR
  ...
  groups_parties -- "1" --> community_admin
...
| `reputation_factions` Reputation & factions | 2 (+6 parts) | director | Factions, standing tiers, sources, unlocks, hostility, alignment |
| `player_alignment` Player alignment | 3 | director | A player-side alignment track |
| `mob_faction_hostility` Mob faction hostility | 3 | director | Standing decides whether mobs attack |
...
16 outbound (a change here reaches out), 34 inbound (a change elsewhere reaches in).
...
| ← in | `group_loot_rules` → `party_loot_rules` | extends | `party choice` | hard | The party picks one of the loot rules |
| ← in | `raid_size_scaling` → `raid_groups` | reads | `size` | hard | Raid size feeds scaling |
...
| ← in | `enemy_factions_hostility` → `mob_faction_hostility` | reads | `mob factions` | hard | Mobs need factions first |
| ← in | `perception` → `mob_faction_hostility` | reads | `attack decision` | hard | The AI's perception consults hostility |
...
| ← in | `server_config_password` → `server_rules_config` | reads | `server settings` | hard | Rules are server settings |
