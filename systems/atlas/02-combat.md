...
- `spell_effect_content` **Spells & effects content** [P0][spec][content-smith] — The SpellDef and StatusEffectDef files the casting and effect systems execute · `data/spells/` +1
  ...
  - `element_matrix` **Element matrix** [P—][candidate][content-smith] — Which element beats which, as data · `docs/balance_ranges.md`
...
|---|---|---|---|
| `element_matrix` Element matrix | 3 | content-smith | Which element beats which, as data |
...
54 outbound (a change here reaches out), 53 inbound (a change elsewhere reaches in).
...
| ← in | `energy_pool` → `resource_cost_check` | reads | `current energy` | soft | Only if energy is approved and the enum extended |
| ← in | `rage_pool` → `resource_cost_check` | reads | `current rage` | soft | Only if rage is approved |
| ← in | `focus_pool` → `resource_cost_check` | reads | `current focus` | soft | Only if focus is approved |
| ← in | `divinity_pool` → `resource_cost_check` | reads | `current divinity` | soft | Only if divinity is approved |
| ← in | `corruption_pool` → `resource_cost_check` | reads | `current corruption` | soft | Only if corruption is approved |
