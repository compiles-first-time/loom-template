...
[P2][spec][content-smith] · Items, loot, crafting, professions, currencies, trade, storage
...
- `loot` **Loot** [P2][spec][content-smith] — Weighted tables, guaranteed drops, rolls on death, chests, group rules · `data/loot_tables/` +1
  ...
  - `world_chests_containers` **World chests** [P—][candidate][world-builder] — Placed containers with loot tables and respawn · `scenes/prefabs/chests/`
  ...
  - `loot_economy_tuning` **Loot economy tuning** [P2][implied][content-smith] — Drop rates and material flow as balance numbers · `docs/balance_ranges.md`
...
- `storage_logistics` **Storage & logistics** [P3][implied][orchestrator] — Chests, shared storage, transfer rules, carts · `core/inventory/containers.gd` +1
  - `chests_containers` **Chests & containers** [P3][implied][orchestrator] — Placed storage with its own inventory, saved with the world · `core/inventory/containers.gd`
...
| `world_chests_containers` World chests | 3 | world-builder | Placed containers with loot tables and respawn |
...
48 outbound (a change here reaches out), 54 inbound (a change elsewhere reaches in).
...
| → out | `recipe_defs` → `cooking` | reads | `station campfire` | hard | Cooking is a set of recipes at the campfire station |
...
| ← in | `timers_cooldowns` → `world_chests_containers` | reads | `respawn` | soft | Chests can refill on a timer |
...
| ← in | `id_convention` → `profession_defs` | reads | `profession ids` | hard | Professions would carry ids like all content |
| ← in | `resource_nodes` → `gathering_professions` | reads | `node harvests` | hard | Gathering skill grows by harvesting |
| ← in | `id_convention` → `currency_defs` | reads | `currency ids` | hard | Currencies would carry ids like all content |
...
| → out | `group_loot_rules` → `party_loot_rules` | extends | `party choice` | hard | The party picks one of the loot rules |
...
| → out | `loot` → `g2_data_integrity` | validates | `loot tables` | hard | G2 checks every table |
| → out | `crafting` → `g2_data_integrity` | validates | `recipes` | hard | G2 checks every recipe |
| → out | `crafting` → `g4_bot_playtest` | validates | `craft` | hard | The bot crafts |
