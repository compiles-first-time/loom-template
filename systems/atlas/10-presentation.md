...
- `input` **Input** [P1][spec][orchestrator] — The input map, translation to intents, rebinding, and candidate gamepad and buffering · `project.godot` +1
  ...
  - `input_sampling` **Per-frame input sampling** [P—][candidate][orchestrator] — Input read every render frame; presses latched into the next tick's intent so nothing is dropped between ticks; look input applied per frame, never quantized to ticks (ADR-0068) · `core/commands/input.gd` +1
...
|---|---|---|---|
| `input_sampling` Per-frame input sampling | 3 | orchestrator | Input read every render frame; presses latched into the next tick's intent so nothing is dropped between ticks; look input applied per frame, never quantized to ticks (ADR-0068) |
...
32 outbound (a change here reaches out), 115 inbound (a change elsewhere reaches in).
...
| ← in | `render_interpolation` → `third_person_rig` | reads | `interpolated target transform` | hard | The camera follows the interpolated transform every frame, never the raw tick position |
| ← in | `render_interpolation` → `locomotion_blending` | reads | `frame-rate playback` | soft | Blends play at frame rate over interpolated motion; tick-locked playback looks choppy |
...
| → out | `third_person_rig` → `g6_feel_gate` | validates | `frame pacing` | hard | The gate measures frame time and hitches during the camera soak |
