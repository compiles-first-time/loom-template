...
- `art_pipeline` **Art pipeline** [P1][spec][world-builder] — The import hook, the master shader, the art bible, naming, placeholders, budgets · `art/` +1
  ...
  - `lod_texture_budgets` **LOD & texture budgets** [P1][implied][world-builder] — Polygon and texture budgets per asset class · `art/`
  - `asset_generation_intake` **Asset generation intake** [P2][spec][world-builder] — Generated assets enter only through the import pipeline, never hand-placed · `art/_inbox/`
- `animation` **Animation** [P1][implied][world-builder/orchestrator] — Rigs, state machines, blending, combat animations, sim-driven timing · `actors/` +1
  ...
  - `animation_state_machines` **Animation state machines** [P1][implied][orchestrator] — AnimationTree states driven by simulation state · `actors/`
  - `locomotion_blending` **Locomotion blending** [P1][implied][world-builder] — Idle, walk, run and jump blends · `actors/`
  - `combat_animations` **Combat animations** [P1][implied][world-builder] — Swings, casts, reactions and deaths · `actors/`
  - `hit_reactions_death_anims` **Hit reactions & deaths** [P1][implied][world-builder] — Reactions triggered by damage and death events · `actors/`
  - `sim_driven_timing` **Sim-driven timing** [P1][spec][orchestrator] — Animation follows simulation timing, never the reverse · `actors/`
  - `ik_procedural` **IK & procedural animation** [P—][candidate][world-builder] — Foot IK and look-at · `actors/`
  - `facial_emotes` **Facial & emote animation** [P—][candidate][world-builder] — Emote and face animations · `actors/`
...
- `audio` **Audio** [P1][implied][world-builder] — SFX, ambience, mix buses, and candidate music · `audio/`
  ...
  - `spatial_audio_mix_buses` **Spatial audio & buses** [P1][implied][world-builder] — 3D positioning and the bus layout · `audio/`
...
- `camera` **Camera** [P1][spec][orchestrator] — The third-person rig and its behaviors · `actors/player/camera/`
  - `third_person_rig` **Third-person rig** [P1][spec][orchestrator] — Follow camera with orbit · `actors/player/camera/`
  - `camera_collision` **Camera collision** [P1][implied][orchestrator] — Pulls in when blocked by geometry · `actors/player/camera/`
  - `zoom_orbit` **Zoom & orbit** [P1][implied][orchestrator] — Zoom and orbit controls · `actors/player/camera/`
...
- `ui_screens` **Screens & menus** [P2][implied][orchestrator] — Full-screen panels: menu, inventory, character, crafting, journal, map, dialogue, settings, join, death, chat · `ui/screens/`
  ...
  - `quest_journal_screen` **Quest journal** [P2][spec][orchestrator] — Active and completed quests with journal text · `ui/screens/journal/`
  ...
  - `tooltips_item_cards` **Tooltips & item cards** [P2][implied][orchestrator] — Item cards with rarity color and comparison · `ui/tooltips/`
...
- `ux_accessibility` **UX & accessibility** [P—][candidate][director] — Colorblind modes, text scaling, subtitles, motion reduction · `ui/theme/`
  ...
  - `subtitles` **Subtitles** [P—][candidate][orchestrator] — Captions for dialogue and barks · `ui/`
...
flowchart LR
  ...
  art_pipeline -- "1" --> spell_effect_content
  art_pipeline -- "2" --> items
...
27 outbound (a change here reaches out), 109 inbound (a change elsewhere reaches in).
...
| → out | `import_hook` → `actor_prefabs_scenes` | reads | `meshes and collision` | hard | Prefab meshes pass through the import hook |
...
| → out | `import_hook` → `collision_generation_import` | extends | `collision step` | hard | Collision generation is one step of the import hook |
...
| ← in | `asset_intake_pipeline` → `asset_generation_intake` | reads | `generator flow` | hard | Intake is the receiving end of the generation pipeline |
...
| ← in | `collision_layers` → `camera_collision` | reads | `camera layer` | hard | The camera probes the world |
...
| ← in | `graphics_presets` → `settings_screens` | reads | `presets` | hard | The graphics tab offers presets |
...
| ← in | `item_comparison` → `tooltips_item_cards` | renders | `deltas` | hard | Cards show comparison deltas |
