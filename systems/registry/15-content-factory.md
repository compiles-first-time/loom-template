...
| converter_gate_run | Converter and gate run | 3 | agent_content_loop | 2 | spec | content-smith | data/_inbox/; data/ | §7.2 | Run the converter, then G2, then G1 | — |
| pr_review_flow | PR review flow | 3 | agent_content_loop | 2 | spec | orchestrator | .github/pull_request_template.md | §7.1 | The PR with .tres files, inbox JSON removed, ids listed, gates pasted | — |
...
| loot_economy_sim | Loot economy simulation | 3 | balance_tuning | — | candidate | test-pilot | tools/testing/balance/loot_sim.gd | — | Monte Carlo over loot tables | — |
| combat_sim_harness | Combat simulation harness | 3 | balance_tuning | — | candidate | test-pilot | tools/testing/balance/combat_sim.gd | — | Headless fights over many seeds | — |
...
| icon_placeholder_fallback | Icon placeholder fallback | 3 | asset_generation | 0 | spec | content-smith | art/icons/_placeholder.png | §7.2 | Use the placeholder until the icon exists | — |
...
| id_listing_in_pr | Id listing in PR | 3 | content_review | 0 | spec | orchestrator | .github/pull_request_template.md | §7.1 | New ids listed in every content PR | — |
| flaky_test_policy | Flaky test policy | 3 | content_review | 0 | spec | test-pilot | .github/workflows/gates.yml; tests/ | §7.2 | A test that would not pass 100 of 100 does not merge | — |
...
| hotfix_data_only | reads | content_database_git | — | hard | A data-only hotfix is a data/ commit |
| lore_voice_check | reads | lore_additions | — | soft | New lore is checked against the voice |
| pr_review_flow | reads | tech_debt_log | — | soft | R9: unrelated debt is filed, not fixed |
| asset_intake_pipeline | reads | icon_generation_prompts | — | hard | Filed prompts are what asset generation picks up |
| pr_review_flow | reads | id_listing_in_pr | — | hard | The PR template requires the id listing |
