...
console.log("\nspec §5 cross-check (R-EB1)");
{
  ...
  assert(v.info.some((i) => /candidate signal\(s\) await a §5 entry.*level_up/.test(i)), "candidate signals are listed as §5 proposals");
...
console.log("\nlive registry (systems/registry)");
{
  ...
  if (!existsSync(dir)) {
    ...
    const a1 = renderAll(live.graph, live.validation, { hash: live.hash });
    const a2 = renderAll(live.graph, live.validation, { hash: live.hash });
    assert([...a1.keys()].every((k) => a1.get(k) === a2.get(k)) && a1.size === a2.size, `rendering is deterministic across ${a1.size} generated files`);
    assert(a1.size === 2 + stats(live.graph).domains.length, "one page per domain plus the index and the explorer");
