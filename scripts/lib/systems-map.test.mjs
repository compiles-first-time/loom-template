...
console.log("\nlive registry (systems/registry)");
{
  ...
  if (!existsSync(dir)) {
    ...
    const a1 = renderAll(live.graph, live.validation, { hash: live.hash, runbooks: live.runbooks, codeowners: live.codeowners });
    const a2 = renderAll(live.graph, live.validation, { hash: live.hash, runbooks: live.runbooks, codeowners: live.codeowners });
    assert([...a1.keys()].every((k) => a1.get(k) === a2.get(k)) && a1.size === a2.size, `rendering is deterministic across ${a1.size} generated files`);
    assert(a1.size === 2 + stats(live.graph).domains.length + 5 + (live.codeowners ? 1 : 0), "one page per domain plus the index, the explorer, the five LLM-pack files and CODEOWNERS when a mapping exists");
