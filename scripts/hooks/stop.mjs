...
import { computeSessionCompliance, complianceSummary } from "../lib/session-compliance.mjs";
import { loadRoster } from "../lib/skill-adherence.mjs";
import { parseJsonl } from "../lib/event-log-read.mjs";
...
//
// The row below is descriptive; description is not a grade. This computes the
// deterministic verdict — was the discipline held? — from records the hooks
// already emitted, and emits it as its own event so the Observatory and
// `loom doctor` can read a *measured* adherence rate instead of trusting that
// the nag was heeded. No inference: the thing that measures whether the model
// behaved must not itself be a model.
//
// Best-effort by design. A fault in the grader must never break Stop, or the
// audit trail loses a session_end to gain a verdict — a strictly worse trade.
let complianceNote = null;
try {
  const roster = await loadRoster();
  const verdict = computeSessionCompliance({
    records: parseJsonl(logText),
    sessionId,
    roster,
  });
  appendEvent(
    mechanicalRecord("session_compliance", {
      session_id: sessionId,
      grade: verdict.grade,
      checks: verdict.checks.map((c) => ({ name: c.name, level: c.level, ok: c.ok, detail: c.detail })),
      adherence_rate: verdict.adherence.adherence_rate,
      suggestions_matched: verdict.adherence.matched,
      suggestions_declined: verdict.adherence.declined,
      suggestions_unmatched: verdict.adherence.unmatched,
      rule: "ADR-0059",
    })
  );
  if (verdict.grade !== "pass") complianceNote = complianceSummary(verdict);
} catch (err) {
  appendEvent(
    mechanicalRecord("session_compliance_error", {
      session_id: sessionId,
...
async function appendSessionRow({
  ...
  // The ledger is gitignored (PR #86 stopped tracking hook-generated session
  // artifacts) and nothing creates it — so an `existsSync → return` here meant
  // that in ANY fresh clone the Session log was never written at all, silently.
  // A closing-the-books checkpoint that no-ops on every new machine is exactly
  // the silent-degradation class ADR-0054 is about, so create the file instead
  // of skipping. Found 2026-08-13 while wiring the ADR-0059 compliance note,
  // which would otherwise have gone nowhere on a fresh clone.
  if (!existsSync(ledgerPath)) {
    try {
      await fs.mkdir(path.dirname(ledgerPath), { recursive: true });
      await fs.writeFile(
        ledgerPath,
        "# Progress ledger\n\n> Generated by the Stop hook (gitignored). Per [L5](../layers/L5-orchestration.md).\n",
        "utf8"
      );
    } catch {
      return; // Read-only checkout or similar — never fail Stop over the ledger.
    }
  }
