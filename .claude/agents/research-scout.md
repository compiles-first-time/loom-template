...
4. **Source-tier filter (mandatory stage 1).** Assign `source_tier` 1/2/3/rejected per [L7 §Source tiering](../../layers/L7-extension.md#source-tiering) + ADR-0009. **Rejected-tier items are dropped here — never written to the inbox.** A feed's `expected_tier` is a prior, not a verdict — judge each item.
